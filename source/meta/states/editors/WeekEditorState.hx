...
class WeekEditorState extends MusicBeatState
{
	...
	function addWeekUI() {
		...
		tab_group.add(new FlxText(backgroundInputText.x, backgroundInputText.y - 18, 0, 'Banner Asset:'));
	...
	function reloadAllShit() {
		...
		backgroundInputText.text = weekFile.weekBanner;
	...
	function reloadBG() {
		...
		var assetName:String = weekFile.weekBanner;
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>) {
		if(id == FlxUIInputText.CHANGE_EVENT && (sender is FlxUIInputText)) {
			if(sender == weekFileInputText) {
				...
				weekFile.weekBanner = backgroundInputText.text.trim();
				reloadBG();

...
// thx weekly for being open source!! <3
...
class UpdatingState extends MusicBeatState
{
    ...
    public function install() {
        ...
        sWfolder = sWfolder.split("FNFTweenly.exe")[0];
    ...
    public function showProgress(event:ProgressEvent) {
        ...
        //trace(johhny + " / " + round);
        if (progN >= 0) {
            progTxt.text = "Downloading Your New Tween: " + progN + "%";
        }
    }
	...
	// Heres The link to change - cross
        var http = new haxe.Http("https://raw.githubusercontent.com/x8c8r/FNF-Tweenly-Public/main/VERSION.txt"); //we gonna have to change this to a custom repo eventually
        ...
        sWfolder = sWfolder.split("FNFTweenly.exe")[0];
        sWfolderB = Std.string(sWfolder);
        trace("Folder Tweenly is Currently Stored in is :" + sWfolder);
        ...
        file = new File(sWfolder + "FNF.Tweenly.zip");
        ...
        progTxt = new FlxText(0, 0, FlxG.width, "Downloading Your New Tween");

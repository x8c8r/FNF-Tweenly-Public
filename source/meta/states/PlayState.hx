...
{
	public static var gameOverVid:String = '';
	...
	override public function create()
	{
		...
		setOnScripts('songCard', songCard);
	...
	function doDeathCheck(?skipHealthCheck:Bool = false) {
		if (((skipHealthCheck && instakillOnMiss) || health <= 0) && !practiceMode && !isDead)
		{
			...
			if(ret != Globals.Function_Stop) {
				...
				if (gameOverVid.length < 1)
					openSubState(new GameOverSubstate(boyfriend.getScreenPosition().x - boyfriend.positionArray[0], boyfriend.getScreenPosition().y - boyfriend.positionArray[1], camFollowPos.x, camFollowPos.y));
				else
					openSubState(new VideoGameOverSubstate());
		...
		if(ret != Globals.Function_Stop && !transitioning) {
			...
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					Init.SwitchToPrimaryMenu(TweenlyMainMenu);
